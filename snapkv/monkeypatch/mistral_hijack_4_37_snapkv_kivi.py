...
from snapkv.monkeypatch.snapkv_utils_kivi_comp import init_snapkv
...
def _manual_attn_with_optional_quant_dropped(self, query_states, dense_key_states, dense_value_states, dropped_cache):
    """
    query_states/dense_key_states/dense_value_states: [B,H,T,D] after RoPE and repeat_kv.
    dropped_cache: SnapKVKIVIDroppedCache or None. Its tensors are dequantized transiently.

    This is a correctness-first PyTorch path. It saves cache memory because dropped tokens are stored quantized,
    but it does not provide KIVI's fused CUDA speedup. For speed, replace dequant+matmul with KIVI CUDA kernels.
    """
    bsz, num_heads, q_len, head_dim = query_states.shape
    dtype = query_states.dtype
    if dropped_cache is not None and dropped_cache.dropped_len > 0:
        qtz = self.kv_cluster.quantizer
        k_drop = qtz.dequantize_key_per_channel(dropped_cache.key_q, dtype=dtype)
        v_drop = qtz.dequantize_value_per_token(dropped_cache.value_q, dtype=dtype)
        all_k = torch.cat([k_drop, dense_key_states], dim=2)
        all_v = torch.cat([v_drop, dense_value_states], dim=2)
    else:
        all_k, all_v = dense_key_states, dense_value_states

    attn_weights = torch.matmul(query_states, all_k.transpose(2, 3)) / math.sqrt(head_dim)
    # Decode path normally has q_len=1, so every cached token is visible. If q_len>1, add a causal mask.
    if q_len > 1:
        kv_len = all_k.shape[-2]
        causal = torch.full((q_len, kv_len), torch.finfo(attn_weights.dtype).min, device=attn_weights.device)
        # This fallback is conservative. Prefill usually uses flash-attn before cache compression.
    ...
    attn_output = torch.matmul(attn_weights, all_v)
