...
from snapkv.monkeypatch.snapkv_utils_scoreaware_kivi import init_snapkv, _tensor_nbytes, format_bytes, KIVIUniformQuantizer
...
def _manual_attn_with_optional_quant_dropped(self, query_states, dense_key_states, dense_value_states, dropped_cache):
    ...
    OOM-safe exact decode attention over:
        score-aware multi-tier quantized dropped KV + dense retained KV.

    It supports tiers with different K/V bitwidth and group size, e.g.:
        QLevel1: K2 G32, V2 G32
        QLevel2: K1 G64, V1 G64

    The implementation uses a numerically stable two-pass softmax and never materializes
    cat([dequantized_dropped, dense]) K/V, so it remains memory-safe for long contexts.
    """
    ...
    if q_len != 1:
        raise RuntimeError(
            "SnapKV+score-aware KIVI OOM-safe path currently expects q_len=1 during decoding. "
    ...
    tiers = getattr(dropped_cache, "tiers", None)
    if tiers is None:
        # Compatibility with very old single-tier cache objects.
        tiers = [dropped_cache]

    # Pass 1: global max logits across all quantized tiers and dense retained KV.
    max_logits = None  # [B,H,1,1], fp32
    for tier in tiers:
        dropped_len = int(tier.dropped_len)
        if dropped_len <= 0:
            continue
        k_qtz = KIVIUniformQuantizer(bits=int(tier.k_bits), group_size=int(tier.k_group_size))
        for st in range(0, dropped_len, chunk_size):
            ed = min(st + chunk_size, dropped_len)
            k_chunk = k_qtz.dequantize_key_per_channel_range(tier.key_q, st, ed, dtype=dtype)
    ...
    # Pass 2: accumulate exp(logits-max)*V and denom over all tiers + dense cache.
    ...
    for tier in tiers:
        ...
        if dropped_len <= 0:
            continue
        k_qtz = KIVIUniformQuantizer(bits=int(tier.k_bits), group_size=int(tier.k_group_size))
        v_qtz = KIVIUniformQuantizer(bits=int(tier.v_bits), group_size=int(tier.v_group_size))
        for st in range(0, dropped_len, chunk_size):
            ed = min(st + chunk_size, dropped_len)
            k_chunk = k_qtz.dequantize_key_per_channel_range(tier.key_q, st, ed, dtype=dtype)
            v_chunk = v_qtz.dequantize_value_per_token_range(tier.value_q, st, ed, dtype=dtype)
            logits = torch.matmul(query_states, k_chunk.transpose(2, 3)) * scale
